...
var run_test = function(
  ...
  if (concurrent && number_of_times % concurrent_batch_size > 0)
    return callback('Number of iterations must be divisible by batch size');
...
var run_single_test = function(
  ...
  test.setup(function(err) {
    ...
    if (!concurrent) {
      ...
      // Warm up the JIT
      exceute_test_serially(func_name, test, warm_up_iterations, [], function(err) {
        if (err) return callback(err, null);
...
var execute_test_batches = function(
  func_name,
  test,
  number_left_to_run_batch,
  number_of_batches,
  results,
  callback
) {
