...
function handleInitialConnectEvent(topology, event) {
  return function(err) {
    var server = this;
    // Debug log
    if (topology.s.logger.isDebug()) {
      topology.s.logger.debug(
        f(
    ...
    // Destroy the instance
    if (topology.state === DESTROYED || topology.state === UNREFERENCED) {
      return this.destroy({ force: true });
    }
    ...
    if (event === 'connect') {
      // Destroy the instance
      if (topology.state === DESTROYED || topology.state === UNREFERENCED) {
        return server.destroy({ force: true });
      }

      // Update the state
      var result = topology.s.replicaSetState.update(server);
      if (result === true) {
        // Primary lastIsMaster store it
        if (server.lastIsMaster() && server.lastIsMaster().ismaster) {
        ...
        if (topology.s.logger.isDebug()) {
          topology.s.logger.debug(
            f(
              ...
              server.name,
              topology.id,
              JSON.stringify(topology.s.replicaSetState.set)
            )
          );
        }
        ...
        if (shouldTriggerConnect(topology)) {
          // We are connected
          topology.state = CONNECTED;

          // Set initial connect state
          topology.initialConnectState.connect = true;
          // Emit connect event
          process.nextTick(function() {
            topology.emit('connect', err, topology);
          });
        ...
        server.destroy({ force: true });
        topology.destroy({ force: true });
        return topology.emit('error', result);
      } else {
        server.destroy({ force: true });
      }
    } else {
      ...
      addServerToList(topology.s.connectingServers, this);
      // Remove from the state
      topology.s.replicaSetState.remove(this);
    }

    if (
      topology.initialConnectState.connect &&
      !topology.initialConnectState.fullsetup &&
      topology.s.replicaSetState.hasPrimaryAndSecondary()
    ) {
      // Set initial connect state
      topology.initialConnectState.fullsetup = true;
      topology.initialConnectState.all = true;
    ...
    for (let i = 0; i < topology.s.connectingServers.length; i++) {
      if (topology.s.connectingServers[i].equals(this)) {
        topology.s.connectingServers.splice(i, 1);
      }
    }

    // Trigger topologyMonitor
    if (topology.s.connectingServers.length === 0 && topology.state === CONNECTING) {
      topologyMonitor(topology, { haInterval: 1 });
    }
  };
}
...
ReplSet.prototype.logout = function(dbName, callback) {
  ...
  if (this.authenticating) {
    throw new MongoError('Authentication or logout already in progress');
  }
