...
function handleInitialConnectEvent(self, event) {
  return function() {
    ...
    if (event === 'connect') {
      ...
      maybeAuthenticateServer(self, _this, function() {
  ...
  var _handleEvent = function(self, event) {
    return function() {
      ...
      if (event === 'connect' && !self.authenticating) {
        // Do we have authentication contexts that need to be applied
        maybeAuthenticateServer(self, _self, function() {
          // Destroyed
          if (self.state === DESTROYED || self.state === UNREFERENCED) {
            moveServerFrom(self.connectingProxies, self.disconnectedProxies, _self);
...
function maybeAuthenticateServer(self, server, callback) {
  if (self.s.credentials == null) {
    return callback();
  }

  server.auth(self.s.credentials, callback);
}
