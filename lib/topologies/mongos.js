...
function handleInitialConnectEvent(topology, event) {
  return function() {
    var server = this;
    ...
    if (topology.state === DESTROYED) {
      // Emit the initial topology
      emitTopologyDescriptionChanged(topology);
      // Move from connectingProxies
      moveServerFrom(topology.connectingProxies, topology.disconnectedProxies, this);
      return this.destroy();
    }
    ...
    if (event === 'connect') {
      ...
      topology.ismaster = server.lastIsMaster();

      // Is this not a proxy, remove t
      if (topology.ismaster.msg === 'isdbgrid') {
        // Add to the connectd list
        for (var i = 0; i < topology.connectedProxies.length; i++) {
          if (topology.connectedProxies[i].name === server.name) {
            // Move from connectingProxies
            moveServerFrom(topology.connectingProxies, topology.disconnectedProxies, server);
            // Emit the initial topology
            emitTopologyDescriptionChanged(topology);
            server.destroy();
            return topology.emit('failed', server);
          }
        }

        // Remove the handlers
        for (i = 0; i < handlers.length; i++) {
          server.removeAllListeners(handlers[i]);
        }

        // Add stable state handlers
        server.on('error', handleEvent(topology, 'error'));
        server.on('close', handleEvent(topology, 'close'));
        server.on('timeout', handleEvent(topology, 'timeout'));
        server.on('parseError', handleEvent(topology, 'parseError'));

        // Move from connecting proxies connected
        moveServerFrom(topology.connectingProxies, topology.connectedProxies, server);
        // Emit the joined event
        topology.emit('joined', 'mongos', server);
      } else {
        // Print warning if we did not find a mongos proxy
        if (topology.s.logger.isWarn()) {
          var message = 'expected mongos proxy, but found replicaset member mongod for server %s';
          // We have a standalone server
          if (!topology.ismaster.hosts) {
            message = 'expected mongos proxy, but found standalone mongod for server %s';
          }

          topology.s.logger.warn(f(message, server.name));
        }

        // This is not a mongos proxy, remove it completely
        removeProxyFrom(topology.connectingProxies, server);
        // Emit the left event
        topology.emit('left', 'server', server);
        // Emit failed event
        topology.emit('failed', server);
      }
    } else {
      moveServerFrom(topology.connectingProxies, topology.disconnectedProxies, this);
      // Emit the left event
      topology.emit('left', 'mongos', this);
      // Emit failed event
      topology.emit('failed', this);
    }
    ...
    if (topology.connectingProxies.length === 0) {
      // Emit connected if we are connected
      if (topology.connectedProxies.length > 0 && topology.state === CONNECTING) {
        // Set the state to connected
        stateTransition(topology, CONNECTED);
        // Emit the connect event
        topology.emit('connect', null, topology);
        topology.emit('fullsetup', topology);
        topology.emit('all', topology);
      } else if (topology.disconnectedProxies.length === 0) {
        // Print warning if we did not find a mongos proxy
        if (topology.s.logger.isWarn()) {
          topology.s.logger.warn(
            f('no mongos proxies found in seed list, did you mean to connect to a replicaset')
          );
        }

        // Emit the error that no proxies were found
        return topology.emit('error', new MongoError('no mongos proxies found in seed list'));
      }

      // Topology monitor
      topologyMonitor(topology, { firstConnect: true });
    }
  };
}
...
Mongos.prototype.destroy = function(options) {
  ...
  // Clear out authentication credentials
  this.s.credentials = null;
...
Mongos.prototype.auth = function(credentials, callback) {
  ...
  if (this.authenticating) {
    return callback(new MongoError('Authentication or logout already in progress'));
  }
  ...
  const authenticateServer = server => {
    const authenticateHandler = err => {
      count = count - 1;
      if (err) {
      ...
      if (count === 0) {
        ...
        if (errors.length) {
          this.s.credentials = null;
          return callback(new MongoError({ message: 'Authentication fail', errors }), false);
        }
    ...
    server.auth(credentials, authenticateHandler);
  };

  while (servers.length > 0) {
    authenticateServer(servers.shift());
  }
};
...
Mongos.prototype.logout = function(dbName, callback) {
  ...
  if (this.authenticating) {
    throw new MongoError('Authentication or logout already in progrsess');
  }
