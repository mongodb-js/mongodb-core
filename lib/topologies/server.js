...
var eventHandler = function(self, event) {
  return function(err) {
    ...
    if(event == 'connect') {
      ...
      var queryOptions = { numberToSkip: 0, numberToReturn: -1, checkKeys: false, slaveOk: true };
      // Create a query instance
      if (self.s.compression && self.s.compression.compressors) {
        var query = new Query(self.s.bson, 'admin.$cmd', {ismaster:true, client: self.clientInfo, compression: self.s.compression.compressors}, queryOptions);
      } else {
        var query = new Query(self.s.bson, 'admin.$cmd', {ismaster:true, client: self.clientInfo, compression: []}, queryOptions);
      }      // Get start time
      var start = new Date().getTime();
      // Execute the ismaster query
      self.s.pool.write(query, {
        ...
        if ("result" in result && "compression" in result.result) {
