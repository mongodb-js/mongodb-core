...
var messageHandler = function(self, state) {
  return function(response, connection) {
    try {
      ...
      var docs = response.documents[0], error = null;
      if(docs && docs.writeErrors && docs.writeErrors.length > 0)
        error = MongoError.create(docs.writeErrors[0]);
      state.callbacks.emit(response.responseTo, error, response);
    } catch (err) {
