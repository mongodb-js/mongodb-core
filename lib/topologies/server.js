...
function maybeAuthenticateServer(server, callback) {
  ...
  if (ismaster && ismaster.arbiterOnly) {
    return callback();
  }

  credentials.resolveAuthMechanism(ismaster);
  server.s.pool.auth(credentials, callback);
}
    ...
    if (event === 'connect') {
      ...
      self.s.pool.write(
        ...
        function(err, result) {
          ...
          // Emit connect
          maybeAuthenticateServer(self, err => self.emit('connect', self, err));
        }
      );
    } else if (
      event === 'error' ||
