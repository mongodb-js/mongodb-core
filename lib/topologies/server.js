...
// Supports server
var isSupportedServer = function(response) {
  return response.ismaster && typeof response.ismaster.minWireVersion === 'number' &&
         response.ismaster.maxWireVersion >= 2; // we no longer support pre-2.6
}

//
// createWireProtocolHandler
var createWireProtocolHandler = function(ismaster) {
...
var Server = function(options) {
  ...
  this.s = {
    ...
    , protocol: options.protocol || new TwoSixWireProtocolSupport()
    // Factory overrides
  ...
  getProperty(this, 'protocol', 'protocol', s.options, {});
...
Server.prototype.command = function(ns, cmd, options, callback) {
  ...
  // Build s for each of the instances
...
Server.prototype.insert = function(ns, ops, options, callback) {
  ...
  // Execute write
  return self.s.protocol.insert(self, self.s.ismaster, ns, self.s.bson, self.s.pool, self.s.callbacks, ops, options, callback);
}
...
Server.prototype.update = function(ns, ops, options, callback) {
  ...
  // Execute write
  return self.s.protocol.update(self, self.s.ismaster, ns, self.s.bson, self.s.pool, self.s.callbacks, ops, options, callback);
}
...
Server.prototype.remove = function(ns, ops, options, callback) {
  ...
  ops = Array.isArray(ops) ? ops : [ops];
  // Execute write
  return self.s.protocol.remove(self, self.s.ismaster, ns, self.s.bson, self.s.pool, self.s.callbacks, ops, options, callback);
}
