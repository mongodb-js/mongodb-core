...
class Pool extends EventEmitter {
  constructor(options = {}) {
    ...
    // If connection monitoring is not enabled, then make emit a no-op
    if (!options.enableConnectionMonitoring) {
      this._emitMonitoringEvent = () => {};
    }
  ...
  // Public API
  acquire(callback) {
    this._emitMonitoringEvent(ConnectionAcquisitionStartedEvent);

    if (this.s.isClosed) {
      this._emitMonitoringEvent(ConnectionAcquisitionFailedEvent, 'poolClosed');
  ...
  release(connection, force, callback) {
    ...
    this._emitMonitoringEvent(ConnectionReleasedEvent, connection);
  ...
  _acquisitionHandler(callback, err, connection) {
    if (!err) {
      ...
      this._emitMonitoringEvent(ConnectionAcquiredEvent, connection);
    ...
    this._emitMonitoringEvent(ConnectionAcquisitionFailedEvent, reason);
  ...
  _tryToGetConnection(callback) {
    ...
    if (maxPoolSize <= 0 || this.totalConnectionCount < maxPoolSize) {
      return this._createConnection(callback);
    }
