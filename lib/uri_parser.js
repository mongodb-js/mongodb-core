...
function parseQueryString(query, options) {
  ...
  splitQueryString.forEach(splitOption => {
    const splitKey = splitOption.split('=')[0];
    const normalizedSplitKey = splitKey.toLowerCase();
    const splitValue = splitOption.split('=')[1];

    if (splitValue === '' || splitValue == null) {
      throw new MongoParseError(`Incomplete key value pair for option.`);
    }

    queryKeys.push(splitKey);
    queryValues.push(splitValue);

    if (
      normalizedKeys.indexOf(normalizedSplitKey) !== -1 &&
      duplicateKeys.indexOf(normalizedSplitKey) === -1
    ) {
      duplicateKeys.push(normalizedSplitKey);
    }

    normalizedKeys.push(normalizedSplitKey);
  });

  for (let i = 0; i < queryKeys.length; i++) {
    let value = queryValues[i];

    const normalizedKey = queryKeys[i].toLowerCase();
    if (duplicateKeys.indexOf(normalizedKey) !== -1) {
      const lastIndex = normalizedKeys.lastIndexOf(normalizedKey);

      if (i === lastIndex) {
        value = queryValues[lastIndex];
        console.warn(
          `Multiple values were provided for ${normalizedKey}. The last value [${value}] will be used.`
        );
      }
    }
