...
const URI_OPTIONS = {
  ...
  serverselectiontryonce: { type: 'boolean' },
  slaveok: { type: 'boolean' },
  slave_ok: { type: 'boolean' },
  sockettimeoutms: { type: 'number' },
  ssl: { type: 'boolean' },
  sslvalidate: { type: 'boolean' },
  tls: { type: 'boolean' },
  tlsallowinvalidcertificates: { type: 'boolean' },
  tlsallowinvalidhostnames: { type: 'boolean' },
  tlscafile: { type: 'string' },
  tlscertificatekeyfile: { type: 'string' },
  tlscertificatekeyfilepassword: { type: 'string' },
  tlsinsecure: { type: 'boolean' },
  w: { type: 'number' },
  waitqueuetimeoutms: { type: 'number' },
  wtimeoutms: { type: 'number' },
  zlibcompressionlevel: { type: 'number' }
};
...
function parseQueryStringItemValue(key, value) {
  if (Array.isArray(value)) {
    ...
    !Number.isNaN(value) &&
    URI_OPTIONS.hasOwnProperty(key) &&
    URI_OPTIONS[key].type !== 'string'
  ) {
...
function applyConnectionStringOption(obj, key, value, options) {
  ...
  if (URI_OPTIONS.hasOwnProperty(key) && URI_OPTIONS[key].type === 'boolean') {
...
function parseQueryString(query, options) {
  ...
  const result = {};
  const queryStringOptions = new Map();
  let splitQueryString = query.split('&');

  splitQueryString.forEach(providedOption => {
    const splitOption = providedOption.split('=');
    const normalizedKey = splitOption[0].toLowerCase();
    ...
    if (queryStringOptions.has(normalizedKey)) {
      let existingValueArray = queryStringOptions.get(normalizedKey);
      existingValueArray.push(value);
      queryStringOptions.set(normalizedKey, existingValueArray);
    } else {
      queryStringOptions.set(normalizedKey, [value]);
    }
  });

  queryStringOptions.forEach((valueArray, optionName) => {
    let parsedValue;
    if (
      URI_OPTIONS.hasOwnProperty(optionName) &&
      URI_OPTIONS[optionName]['type'] !== 'array' &&
      valueArray.length > 1
    ) {
      const lastValue = valueArray[valueArray.length - 1];
      console.warn(
        `Multiple values were provided for ${optionName}. The last value [${lastValue}] will be used.`
      );
      parsedValue = parseQueryStringItemValue(optionName, lastValue);
    } else {
      if (valueArray.length === 1) {
        parsedValue = parseQueryStringItemValue(optionName, valueArray[0]);
      } else {
        parsedValue = parseQueryStringItemValue(optionName, valueArray);
      }
    }

    applyConnectionStringOption(result, optionName, parsedValue, options);
  });
