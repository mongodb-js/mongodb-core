...
function checkTLSOptions(queryString) {
  ...
  const queryStringHasTLS = queryStringKeys.indexOf('tls') !== -1;
  const queryStringHasSSL = queryStringKeys.indexOf('ssl') !== -1;

  if (queryStringHasTLS && queryStringHasSSL) {
    let tlsValue;
    if (Array.isArray(queryString['tls'])) {
      tlsValue = queryString['tls'][0];
    } else {
      tlsValue = queryString['tls'];
    }

    let sslValue;
    if (Array.isArray(queryString['ssl'])) {
      sslValue = queryString['ssl'][0];
    } else {
      sslValue = queryString['ssl'];
    }
  ...
  if (queryStringHasTLS) {
    checkAllOptions('tls', queryString);
  }

  if (queryStringHasSSL) {
    checkAllOptions('ssl', queryString);
  }
}
 ...
 * @throws {MongoParseError}
 */
function checkAllOptions(key, queryString) {
  if (Array.isArray(queryString[key])) {
    const firstValue = queryString[key][0];
    queryString[key].forEach(tlsValue => {
      if (tlsValue !== firstValue) {
        throw new MongoParseError('All values of ${key} must be the same.');
      }
    });
  }
}
