...
class Server extends EventEmitter {
  ...
  destroy(options, callback) {
    ...
    // symmetrically stop listening to all events on the pool
...
function parseErrorEventHandler(server) {
  return function(error) {
    // These are errors emitted when the driver is unable to complete parsing the wire
    // protocol envelope. Per SDAM we MUST transition the server to an `Unknown` state,
    // and reset the connection pool.

    server.s.pool.reset(() => {
      // cancel monitoring, if needed
      if (server.s.monitorId) {
        clearTimeout(server.s.monitorId);
        server.s.monitorId = null;
