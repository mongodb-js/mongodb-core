...
class Server extends EventEmitter {
  ...
  constructor(description, options) {
    ...
    this.s = {
      ...
      wireProtocolHandler: new TwoSixWireProtocolSupport()
    };
  }
  ...
  // legacy accessor for wire protocol handler
  get wireProtocolHandler() {
    return this.s.wireProtocolHandler;
  }
  ...
  command(ns, cmd, options, callback) {
    ...
    this.s.wireProtocolHandler.command(this, ns, cmd, options, callback);
  }
...
function executeWriteOperation(args, options, callback) {
  ...
  // Execute write
  return server.s.wireProtocolHandler[op](server, ns, ops, options, callback);
}
...
function maybeAuthenticateServer(server) {
  const credentials = server.s.credentials;
  if (credentials == null || server.type === ServerType.RSArbiter) {
    server.s.state = STATE_CONNECTED;
    server.emit('connect', server);
    return;
  }

  credentials.resolveAuthMechanism(server.description);
  server.s.pool.auth(credentials, err => {
    if (err) {
...
function connectEventHandler(server) {
  return function() {
    ...
    executeServerHandshake(server, (err, response) => {
      ...
      maybeAuthenticateServer(server);
    });
  };
}
...
function errorEventHandler(server) {
  return function(err) {
    if (err) server.emit('error', new MongoNetworkError(err));
    server.emit('close');
  };
}
