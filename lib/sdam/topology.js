...
class Topology extends EventEmitter {
  ...
  connect(options, callback) {
    ...
    this.selectServer(readPreferenceServerSelector(readPreference), err => {
      if (err) return callback(err, null);

      // NOTE: legacy event for topology being opened
...
function connectServers(topology, serverDescriptions) {
  topology.s.servers = serverDescriptions.reduce((servers, serverDescription) => {
    ...
    server.once('connect', serverConnectEventHandler(server));
...
function serverConnectEventHandler(server) {
