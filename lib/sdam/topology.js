...
class Topology extends EventEmitter {
  ...
  constructor(seedlist, options) {
    ...
    if (options.srvHost) {
      ...
      this.on('topologyDescriptionChanged', ev => {
        ...
        if (previousType !== TopologyType.Sharded && newType === TopologyType.Sharded) {
          this._handleSrvPolling = this.handleSrvPolling.bind(this);
          this.srvPoller.on('srvPolling', this._handleSrvPolling);
          this.srvPoller.start();
        }
      });
    }
  }
  ...
  handleSrvPolling(ev) {
    ...
    const serversToRemove = new Set();
    for (const server of this.s.description.servers) {
      if (newAddresses.has(server[0])) {
        newAddresses.delete(server[0]);
      } else {
        serversToRemove.add(server[1]);
      }
    }

    const previousTopologyDescription = this.s.description;

    if (newAddresses.size <= 0 && serversToRemove.size <= 0) {
      return;
    }

    for (const address of newAddresses) {
      const serverDescription = new ServerDescription(address); // Creating topology type unknown
      this.s.description = this.s.description.update(serverDescription);
      updateServers(this, serverDescription);
    }

    if (serversToRemove.size > 0) {
      this.s.description = this.s.description.removeServers(serversToRemove);
      for (const entry of this.s.servers) {
        const serverAddress = entry[0];
        const server = entry[1];
        if (!this.s.description.hasServer(serverAddress)) {
          this.s.servers.delete(serverAddress);
  ...
  close(options, callback) {
    ...
    if (this.srvPoller) {
      this.srvPoller.stop();
      if (this._handleSrvPolling) {
        this.srvPoller.removeListener('srvPolling', this._handleSrvPolling);
        delete this._handleSrvPolling;
      }
    }
