...
var Response = function(bson, data, opts) {
  ...
  if (this.opCode == OP_COMPRESSED) {
    ...
    this.opCode = data[this.index] | data[this.index + 1] << 8 | data[this.index + 2] << 16 | data[this.index + 3] << 24;
    this.index = this.index + 4;

    // Read uncompressed size
    var uncompressedSize = data[this.index] | data[this.index + 1] << 8 | data[this.index + 2] << 16 | data[this.index + 3] << 24;
    this.index = this.index + 4;

    // Read compressor ID
    var compressorID = data[this.index];
    this.index = this.index + 1;

    // Read compressed data
    var compressedData = data.slice(this.index)
    switch (compressorID) {
      case 0:
