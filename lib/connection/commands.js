...
var Response = function(bson, data, opts) {
  ...
  if (this.opCode == 2012) {
    ...
    // Reset the index so that this message can be processed as it it had been in its original uncompressed form
    this.index = 0

    // Note on the response that this message was originally compressed (used for testing)
    this.fromCompressed = true
  }
