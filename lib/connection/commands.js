...
var readHeader = require('../wireprotocol/shared').readHeader;
...
Query.prototype.fromBin = function(bson) {
...
var Response = function(bson, data, opts, callback) {
  ...
  // Read the header
  var parsedHeader = readHeader(data);

  this.length = parsedHeader.messageLength;
  this.requestId = parsedHeader.requestId;
  this.responseTo = parsedHeader.responseTo;
  this.opCode = parsedHeader.opCode;

  // Index for parsing the body
  this.index = 16;

  console.log('commands.js:428')

  // If the message we get is OP_COMPRESSED, then we have to uncompress the data
  if (this.opCode == OP_COMPRESSED) {

    console.log('commands.js:432')
    this.opCode = data.readInt32LE(MESSAGE_HEADER_SIZE + 0);
    console.log('commands.js:434')
    var uncompressedSize = data.readInt32LE(MESSAGE_HEADER_SIZE + 4);
    var compressorId = data.readUInt8(MESSAGE_HEADER_SIZE + 8);
    var compressedData = data.slice(MESSAGE_HEADER_SIZE + 9);
    console.log('compressorID %d', compressorId)

    switch (compressorId) {
      case 0:
        console.log('OP_COMPRESSED with no compression.')
        this.raw = compressedData;
        this.data = compressedData;
        this.preParse()
        callback(this)
        break;
      case 1:
        console.log('OP_COMPRESSED with snappy compression.')
        var decompressedData = Snappy.uncompress(compressedData, function (err, decompressedData) {
          console.log('OP_COMPRESSED decompressed using snappy')

          if (err) {
            console.log(err)
          }
          self.raw = decompressedData;
          self.data = decompressedData;
          self.preParse()
          console.log('Back in new Response')
          callback(this)
        });
        break;
      case 2:
        var decompressedData = zlib.unzip(compressedData, function (err, decompressedData) {
          if (err) {
            console.log(err)
          }
          self.raw = decompressedData;
    ...
    console.log('commands.js:470')  
    this.preParse(data.slice(MESSAGE_HEADER_SIZE))
    callback(this)
  }
  
}

Response.prototype.preParse = function() {
  console.log('Start of preparse')
  var index = 0
  // Unpack flags
  this.responseFlags = this.data[index] | this.data[index + 1] << 8 | this.data[index + 2] << 16 | this.data[index + 3] << 24;
  index = index + 4;
  ...
  this.startingFrom = this.data[index] | this.data[index + 1] << 8 | this.data[index + 2] << 16 | this.data[index + 3] << 24;
  index = index + 4;

  // Unpack the number of objects returned
  this.numberReturned = this.data[index] | this.data[index + 1] << 8 | this.data[index + 2] << 16 | this.data[index + 3] << 24;
  index = index + 4;
