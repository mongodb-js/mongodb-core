...
var compress = function(self, dataToBeCompressed, callback) {
  switch (self.options.agreedCompressor) {
    case 'snappy':
      ...
      break;
    case 'zlib':
      zlib.deflate(dataToBeCompressed, {
        level: self.options.zlibCompressionLevel || zlib.constants.Z_DEFAULT_COMPRESSION
      }, callback);
      break;
    default:
    ...
    commandsPromises.push(new Promise(function(resolve, reject) {
      ...
      if (self.options.agreedCompressor && checkForUncompressibleCommands(command)) {
        ...
        compress(self, messageToBeCompressed, function(err, compressedMessage) {
          ...
          msgHeader.fill(concatenatedOriginalCommandBuffer.slice(4, 8), 4, 8) // requestID (extracted from original message)
