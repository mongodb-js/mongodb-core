...
function connectionFailureHandler(self, event) {
  return function(err) {
    //  console.log(`Pool connectionFailureHandler ${event} = ${err}`)
...
function attemptReconnect(self) {
  return function() {
    // //  console.log("-------- attemptReconnect 0")
    ...
    // //  console.log("-------- attemptReconnect 2")

    // If we have failure schedule a retry
    function _connectionFailureHandler(self, event) {
      return function() {
        //  console.log(`-------- attemptReconnect _connectionFailureHandler :: ${event}`)
        if (this._connectionFailHandled) return;
        this._connectionFailHandled = true;
        // Destroy the connection
        this.destroy();
    ...
    function _connectHandler(self) {
      return function() {
        // //  console.log(`-------- attemptReconnect _connectHandler`)
        // Assign
        var connection = this;
...
function messageHandler(self) {
  return function(message, connection) {
    ...
    // //  console.dir(this.workItems)
    // // //  console.dir(connection)
    // //  console.log(`message.responseTo=${message.responseTo}`)
    // //  console.log(`connection.requestId=${connection.workItems.map(x => { return x.requestId; })}`)
    ...
    function authenticateStragglers(self, connection, callback) {
      ...
      // //  console.log("!!!!!!!!!!!! do it 0")
      // //  console.dir(connection.workItems.length)
...
Pool.prototype.connect = function(auth) {
  // //  console.log(`== Pool.prototype.connect 0`)
  ...
  // //  console.log(`== Pool.prototype.connect 1`)
  // Add listeners to the connection
  connection.once('connect', function(connection) {
    // //  console.log(`== Pool.prototype.connect 0:1`)
  ...
  // //  console.log(`== Pool.prototype.connect 2`)
  try {
    connection.connect();
  } catch(err) {
  ...
  // //  console.log(`== Pool.prototype.connect 3`)
}
...
Pool.prototype.write = function(commands, options, cb) {
  ...
  if(Array.isArray(commands)) {
    ...
    for(var i = 0; i < commands.length; i++) {
      // console.dir(commands[i])
      buffer.push(commands[i].toBin());
    }

    // Get the requestId
    operation.requestId = commands[commands.length].requestId;
  } else {
    // console.log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 1")
    // console.dir(commands.toBin)
  ...
  return function() {
    ...
    waitForAuth(function() {
      ...
      while(true) {
        ...
        // console.log("======= chose connection " + (self.connectionIndex++ % self.availableConnections.length))
        ...
        if(connection.isConnected()) {
          ...
          // //  console.log("!!!!!!!!!!!!!!! write operation 0")
          // We have a custom socketTimeout
          if(!workItem.immediateRelease && typeof workItem.socketTimeout == 'number') {
            connection.setSocketTimeout(workItem.socketTimeout);
          }

          // //  console.log("!!!!!!!!!!!!!!! write operation 1")
          ...
          if(totalConnections < self.options.size
            && self.queue.length > 0) {
              //  console.log("============== createConnection :: " + connectionId++)
            // Create a new connection
            _createConnection(self);
            // // Attempt to execute again
            // self.executing = false;
