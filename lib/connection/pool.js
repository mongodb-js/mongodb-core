...
function authenticate(pool, credentials, connection, cb) {
  // If there are no credentials, do not authenticated
  if (!credentials) {
    return cb(null);
  }

  // HACK: If default mechanism has not been determined yet, do not
  // try to authenticate
  ...
  provider.auth.call(provider, write(pool), [connection], credentials, cb);
}
