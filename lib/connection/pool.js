  ...
  for(var i = 0; i < commands.length; i++) {
    ...
    if (self.options.agreedCompressor && checkForUncompressibleCommands(commands[i])) {
      ...
      switch (self.options.agreedCompressor) {
        case 'snappy':
        var compressorID = 1;
          var compressedMessage = Snappy.compressSync(uncompressedMessage);
          break;
        case 'zlib':
          var compressorID = 2;
          var zlibCompressionLevel = self.options.zlibCompressionLevel || zlib.constants.Z_DEFAULT_COMPRESSION;
          var compressedMessage = zlib.gzipSync(uncompressedMessage, {
            level: zlibCompressionLevel
          });
        default:
          var compressorID = 0;
          var compressedMessage = uncompressedMessage;
          break;
      }
      ...
      // Concatenate the OP_COMPRESSED components
      var command = Buffer.concat([msgHeader, compressionDetails, compressedMessage]);
    } else {
      var command = originalCommandBuffer
    }
