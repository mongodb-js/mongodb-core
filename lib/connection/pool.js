...
Pool.prototype.write = function(commands, options, cb) {
  ...
  // For each command, create a promise while we create the command to send to the server
  var commandsPromises = []
  for(var i = 0; i < commands.length; i++) {
    commandsPromises.push(new Promise(function(resolve, reject) {
      var originalCommandBuffer = commands[i].toBin()

      // If a compressor has been agreed and this command does not contain a command that must not be compressed, transform this into OP_COMPRESSED
      if (self.options.agreedCompressor && checkForUncompressibleCommands(commands[i])) {
        ...
        var uncompressedMessage = concatenatedOriginalCommandBuffer.slice(16)

        // Construct an OP_COMPRESSED message from a given compressed message body
        var postCompressionHandler = function(err, compressedMessage) {
          if (err) {
            console.log(err)
          }

          // Create the msgHeader of OP_COMPRESSED
          var msgHeader = new Buffer(16);
          msgHeader.writeInt32LE(16 + 9 + compressedMessage.length, 0)
          msgHeader.fill(concatenatedOriginalCommandBuffer.slice(4, 8), 4, 8)
          msgHeader.fill(0, 8, 12)
          msgHeader.writeInt32LE(2012, 12)

          // Create the compression details of OP_COMPRESSED
          var compressionDetails = new Buffer(9)
          compressionDetails.fill(concatenatedOriginalCommandBuffer.slice(12, 16), 0, 4)
          compressionDetails.writeInt32LE(uncompressedMessage.length, 4)
          compressionDetails.writeUInt8(compressorID, 8)

          // Concatenate the OP_COMPRESSED components and resolve the pending promise
          resolve(Buffer.concat([msgHeader, compressionDetails, compressedMessage]))
        }

        // Compress uncompressedMessage
        switch (self.options.agreedCompressor) {
          case 'snappy':
            var compressorID = 1;
            var compressedMessage = Snappy.compress(uncompressedMessage, postCompressionHandler);
            break;
          case 'zlib':
            var compressorID = 2;
            var zlibCompressionLevel = self.options.zlibCompressionLevel || zlib.constants.Z_DEFAULT_COMPRESSION;
            var compressedMessage = zlib.deflate(uncompressedMessage, { level: zlibCompressionLevel }, postCompressionHandler);
            break;
          default:
            var compressorID = 0;
            postCompressionHandler(null, uncompressedMessage)
          break;
        }
      } else {
        resolve(originalCommandBuffer)
      }
    }));
  }

  // Wait for all the commands to be created
  Promise.all(commandsPromises).then(function(buffer) {
    // Get the requestId
...
var checkForUncompressibleCommands = function(command) {
  var canCompressCommand = true;
  uncompressibleCommands.forEach(function(uncompressibleCommand) {
    if (command.query.hasOwnProperty(uncompressibleCommand)) {
      canCompressCommand = false;
    }
  });
  return canCompressCommand;
}
...
function _hasForcedUncompressedCommands(commands) {
