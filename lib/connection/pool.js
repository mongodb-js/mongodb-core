...
Pool.prototype.write = function(commands, options, cb) {
  ...
  for(var i = 0; i < commands.length; i++) {
    ...
    if (self.options.agreedCompressor) {
      console.log("we have an agreed compressor")
      var canCompressCommand = true;
      uncompressibleCommands.forEach(function(uncompressibleCommand) {
        console.log("Command to be checked", uncompressibleCommand)
        if (commands[i].query.hasOwnProperty(uncompressibleCommand)) { 
          canCompressCommand = false;
        }
      });

      console.log("Can compress command", canCompressCommand)
      if (canCompressCommand) {
        ...
        console.log("Can't compress the command")
