...
var emitMessageHandler = function (self, message) {
  ...
  if (msgHeader.opCode == OP_COMPRESSED) {
    ...
    index++;
    decompress(compressorID, message.slice(index), function(err, decompressedData) {
      if (err) {
        throw err;
      }
    ...
    self.messageHandler(new Response(self.bson, msgHeader, message.slice(MESSAGE_HEADER_SIZE), self.responseOptions), self);
  }
}
