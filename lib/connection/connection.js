...
var inherits = require('util').inherits
  ...
  , OP_COMPRESSED = require('./commands').OP_COMPRESSED;
...
var decompress = function(compressorId, compressedData, callback) {
  var decompressors = [
    function(compressedData, callback) {
  ...
  if (compressorId < 0 || compressorId > decompressors.length - 1) {
    throw new Error('Server sent message compressed using an unsupported compressor');
  }

  // Pass the compressed date to the appropriate decompressor
  decompressors[compressorId](compressedData, function(err, decompressedData) {
    if (err) {
      console.log(err)
    }
...
var emitMessageHandler = function (message, self) {
  var msgHeader = parseHeader(message);
  if (msgHeader.opCode == OP_COMPRESSED) {
    msgHeader.fromCompressed = true;
    msgHeader.opCode = message.readInt32LE(16);
    msgHeader.length = message.readInt32LE(20);
    var compressorId = message[24];

    decompress(compressorId, message.slice(25), function(decompressedData) {
      if (decompressedData.length !== msgHeader.length) {
        throw new Error('Decompressing a compressed message from the server failed. The message is corrupt.')
      }
      self.messageHandler(new Response(self.bson, msgHeader, decompressedData, self.responseOptions), self);
    })
  } else {
    self.messageHandler(new Response(self.bson, msgHeader, message.slice(16), self.responseOptions), self);
  }
}
      ...
      if(self.bytesRead > 0 && self.sizeOfMessage > 0) {
        ...
        if(remainingBytesToRead > data.length) {
          ...
          try {
            ...
            emitMessageHandler(emitBuffer, self);
