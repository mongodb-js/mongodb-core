...
var Connection = function(messageHandler, options) {
  ...
  this.ipv6Connection = null;
  this.ipv4Connection = null;
...
function errorHandlerForIPv6(self) {
  return function(err) {
    if (connectionAccounting) deleteConnection(self.id);
    // Debug information
    if (self.logger.isDebug())
...
function fastFallback(self, family, _options, _errorHandler) {
  const _errorHandlerForIPv6 = errorHandlerForIPv6(self);
  let connected = false;
  if (family === 6 && !self.domainSocket && self.family === void 0) {
    self.ipv6Connection = makeUnsecureConnection(self, 6);
    setupConnection(self, self.ipv6Connection, _errorHandlerForIPv6);

    self.ipv4Connection = makeUnsecureConnection(self, 4);
    setupConnection(self, self.ipv4Connection, _errorHandler);

    self.ipv6Connection.once('connect', function() {
      if (!connected) {
        self.connection = self.ipv6Connection;
        onConnect(self);
        connected = true;
      }
    });
    self.ipv4Connection.once('connect', function() {
      setTimeout(function() {
        if (!connected) {
          self.connection = self.ipv4Connection;
          onConnect(self);
          connected = true;
        }
      }, 250);
...
Connection.prototype.connect = function(_options) {
  ...
  doConnect(this, 6, _options, _errorHandler);
};
