...
function makeSSLConnection(self, _options, _errorHandler) {
  let sslOptions = {
  ...
  setupConnection(self, connection, _errorHandler);
...
function makeUnsecureConnection(self, family) {
  ...
  console.log(`family is ${connection_options.family}`);
...
function fastFallback(self, family, _options, _errorHandler) {
  let connected = false;
  if (family === 123 && !self.domainSocket) {
    const ipv6Connection = makeUnsecureConnection(self, 6);
    setupConnection(self, ipv6Connection, _errorHandler);

    const ipv4Connection = makeUnsecureConnection(self, 4);
    setupConnection(self, ipv4Connection, _errorHandler);

    ipv6Connection.once('connect', function() {
      if (!connected) {
        console.log('6 is connected');
        shutdownConnection(ipv4Connection);
        self.connection = ipv6Connection;
        connected = true;
      }
    });
    ipv4Connection.once('connect', function() {
      console.log('4 is connected');
      self.connection = ipv4Connection;

      setTimeout(function() {
        if (!connected) {
          console.log('4 is chosen');
          shutdownConnection(ipv6Connection);
          self.connection = ipv4Connection;
          connected = true;
        }
      }, 250);
    ...
    self.connection = makeUnsecureConnection(self, family);
    setupConnection(self, self.connection, _errorHandler);
  }
}

function doConnect(self, family, _options, _errorHandler) {
  if (self.ssl) {
    makeSSLConnection(self, _options, _errorHandler);
  } else {
    fastFallback(self, family, _options, _errorHandler);
  }
}

function setupConnection(self, connection, _errorHandler) {
  // Add handlers for events
  connection.once('error', _errorHandler);
...
Connection.prototype.connect = function(_options) {
  ...
  if (this.family !== void 0) {
    console.log('original family: ' + this.family);
  ...
  // });
  return doConnect(this, 6, _options, _errorHandler);
};
