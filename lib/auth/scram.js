...
var passwordDigest = function(username, password, callback) {
  if (typeof username !== 'string') {
    const err = new MongoError('username must be a string');
    if (callback) {
      return callback(err);
    }
    throw err;
  ...
  if (typeof password !== 'string') {
    const err = new MongoError('password must be a string');
    if (callback) {
      return callback(err);
    }
    throw err;
  ...
  if (password.length === 0) {
    const err = new MongoError('password cannot be empty');
    if (callback) {
      return callback(err);
    }
    throw err;
...
class ScramSHA extends AuthProvider {
  ...
  _executeScram(sendAuthCommand, connection, credentials, nonce, callback) {
    ...
    if (cryptoMethod === 'sha256') {
      ...
      processedPassword = passwordDigest(username, password, callback);
    }
