...
var storeSymbol = Symbol("authStore");
/**
    ...
    var execute = function(connection) {
      ...
      GSSAPIInitialize(self, db, username, password, db, gssapiServiceName, server, connection, options, function(err, r) {
        ...
        if(count == 0 && numberOfValidConnections > 0) {
          ...
          addAuthSession(self[storeSymbol], new AuthSession(db, username, password, options));
...
GSSAPI.prototype.logout = function(dbName) {
  this[storeSymbol] = this[storeSymbol].filter(function(x) {
    return x.db != dbName;
  });
}
...
GSSAPI.prototype.reauthenticate = function(server, connections, callback) {
  var authStore = this[storeSymbol].slice(0);
