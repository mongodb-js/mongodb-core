...
class AuthProvider {
  ...
  auth(sendAuthCommand, connections, credentials, callback) {
    return this._auth(sendAuthCommand, connections, credentials, callback);
  }
   ...
   * @ignore
   */
  _auth(sendAuthCommand, connections, credentials, callback) {
    // Total connections
   ...
   * @method
   * @param {SendAuthCommand} sendAuthCommand Topology the authentication method is being called on
   * @param {Connection[]} connections Connections to authenticate using this authenticator
   * @param {authResultCallback} callback The callback to return the result from the authentication
   * @return {object}
   */
  reauthenticate(sendAuthCommand, connections, callback) {
    ...
    for (let i = 0; i < authStore.length; i++) {
      this._auth(sendAuthCommand, connections, authStore[i], function(err) {
        count = count - 1;
        if (count === 0) {
          callback(err, null);
        }
