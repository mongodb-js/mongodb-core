 ...
 */
const collationNotSupported = (server, cmd, callback) => {
  if (cmd && cmd.collation && server.ismaster && server.ismaster.maxWireVersion < 5) {
    callback(new MongoError(f('server %s does not support collation', server.name)));
    return true;
  }
};
